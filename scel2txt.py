...
import fnmatch
import os
def find_files(directory,pattern='*.scel'):
    files = []
    for root,dirnames,filenamses in os.walk(directory):
        for filenamse in fnmatch.filter(filenamses,pattern):
            files.append(os.path.join(root,filenamse))
    return files
